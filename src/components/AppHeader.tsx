...
export async function AppHeader({ user, active }: { user: User; active?: 'trilhas' | 'meus-cursos' | 'comunidade' | 'eventos' | 'perfil' }) {
  ...
  return <>
    <header className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur-md">
      <div className="max-w-[1440px] mx-auto px-5 md:px-8 min-h-20 flex items-center justify-between gap-5">
        <Link href="/" className="shrink-0"><span className="eyebrow !mb-0 !text-foreground !text-base">Sanctum</span><span className="hidden lg:block text-[11px] text-foreground-muted tracking-wider">O Polímata Hermético</span></Link>
        <nav aria-label="Principal" className="hidden md:flex items-center gap-5 lg:gap-8">{links.map((link) => <Link key={link.href} href={link.href} aria-current={active === link.key ? 'page' : undefined} className={active === link.key ? 'text-primary text-sm' : 'text-sm text-foreground-muted hover:text-foreground'}>{link.label}</Link>)}</nav>
        <div className="flex items-center gap-4">
          ...
          <details className="relative">
            <summary className="list-none cursor-pointer w-10 h-10 rounded-full border border-border grid place-items-center text-primary" aria-label="Menu da conta">{user.name.slice(0, 1)}</summary>
