...
export async function CertificateCard({ certificate }: CertificateCardProps) {
    ...
    return (<div className="certificate-print relative p-8 rounded-lg border border-primary/40 bg-gradient-to-br from-surface via-background to-surface overflow-hidden">
      ...
      <div className="relative">
        ...
        <h2 className="text-3xl font-heading font-bold mt-1">{certificate.recipientName || user?.name || '—'}</h2>
        <p className="mt-4 text-foreground-muted">a conclusão da trilha</p>
        <h3 className="text-xl font-heading font-semibold text-primary mt-1">{certificate.courseTitle || course?.title || 'Trilha concluída'}</h3>
