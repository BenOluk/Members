...
export async function PostCard({ post, currentUser, showSpace = false }: PostCardProps) {
    const author = (await getUserById(post.authorId));
    const space = showSpace ? (await getSpaceById(post.spaceId)) : undefined;
    if (!author)
        return null;
    ...
    return (<article className="folio p-5 md:p-7 hover:border-primary/30 transition-colors duration-200 relative">
      ...
      <div className="flex items-start gap-3.5 mb-4">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 leading-none">
            ...
            {author.role === 'admin' && (<span className="text-[9px] uppercase tracking-widest font-bold text-primary border border-primary/30 px-1.5 py-0.5 rounded-sm">
                Grão-Mestre
              </span>)}
            {author.role === 'moderator' && (<span className="text-[9px] uppercase tracking-widest font-bold text-foreground-muted border border-border px-1.5 py-0.5 rounded-sm">
                Mod
              </span>)}
          </div>
      ...
      {post.title && (<h3 className="text-base font-heading font-semibold mb-2 leading-snug">{post.title}</h3>)}
      ...
      <div className="flex items-center gap-5 pt-4 border-t border-border text-[11px] text-foreground-muted font-medium uppercase tracking-wider">
        <form action={toggleLikePost.bind(null, post.id)}>
          <button type="submit" className={`flex items-center gap-1.5 transition-colors duration-200 ${liked ? 'text-primary' : 'hover:text-primary'}`} aria-label={liked ? 'Remover curtida' : 'Curtir post'}>
      ...
      {post.comments.length > 0 && (<div className="mt-4 pt-4 border-t border-border space-y-3.5">
          {(await Promise.all(post.comments.map(async (c) => {
                const cauthor = (await getUserById(c.authorId));
                if (!cauthor)
                    return null;
      ...
      <form action={addComment.bind(null, post.id)} className="mt-4 pt-4 border-t border-border flex gap-3">
        ...
        <input type="text" name="content" required maxLength={2000} placeholder="Responder..." className="flex-1 bg-background border border-border rounded-sm px-3 py-1.5 text-sm focus:outline-none focus:border-primary/50 placeholder:text-foreground-muted/50"/>
        <button type="submit" className="text-[11px] uppercase tracking-wider font-bold text-primary hover:text-primary-hover transition-colors">
          Enviar
        </button>
      </form>
    </article>);
