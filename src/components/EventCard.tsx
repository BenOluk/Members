...
export function EventCard({ event, variant = 'list' }: EventCardProps) {
  ...
  if (variant === 'card') {
    return (
      <a href={event.joinUrl} className="group block rounded overflow-hidden border border-border bg-surface hover:border-primary/40 transition-colors duration-200">
        <div className="relative h-36">
          ...
          <span
            className="absolute top-3 left-3 text-[10px] uppercase tracking-widest font-bold px-2 py-0.5"
            style={{ color: accent }}
          >
            {KIND_LABEL[event.kind]}
          </span>
          {t.isLive && (
            <span className="absolute top-3 right-3 text-[10px] uppercase tracking-widest font-bold px-2 py-0.5 bg-destructive text-foreground animate-pulse">
              ● AO VIVO
            </span>
          )}
        </div>
  ...
  return (
    <a href={event.joinUrl} className="group flex items-center gap-3 p-3 rounded border border-border bg-surface hover:border-primary/40 transition-colors duration-200">
