...
export async function EventCard({ event, variant = 'list' }: EventCardProps) {
    ...
    if (variant === 'card') {
        return (<a href={event.joinUrl} className="group block overflow-hidden folio hover:border-primary/40 transition-colors duration-200">
        <div className="relative h-36">
          ...
          <span className="absolute top-3 left-3 course-plate__meta px-2 py-0.5">
    ...
    return (<a href={event.joinUrl} className="group flex items-center gap-3 p-3 folio hover:border-primary/40 transition-colors duration-200">
      <div className="flex flex-col items-center justify-center w-14 h-14 bg-background border border-border rounded flex-shrink-0">
        <span className="course-plate__meta">
          {KIND_LABEL[event.kind]}
        </span>
