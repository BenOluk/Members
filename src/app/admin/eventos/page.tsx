...
function isPast(e: LiveEvent): boolean {
    return new Date(e.startsAt).getTime() + e.durationMinutes * 60000 < Date.now();
}
function toLocalInput(iso: string): string {
    return new Date(Date.parse(iso) - 3 * 3600000).toISOString().slice(0, 16);
}
function EventForm({ event, hosts }: {
    event?: LiveEvent;
    hosts: Array<{
        id: string;
        name: string;
    ...
    return (<form action={saveEvent} className="grid md:grid-cols-3 gap-3 items-end">
      ...
      <div>
        <label className={labelCls} htmlFor={`ev-start-${key}`}>Início · horário de Brasília</label>
        <input id={`ev-start-${key}`} name="startsAt" type="datetime-local" required defaultValue={event ? toLocalInput(event.startsAt) : ''} className={`${inputCls} w-full`}/>
      </div>
      <div>
...
export default async function AdminEventsPage() {
    ...
    const hosts = (await listUsers()).filter((u) => u.role !== 'student')
    ...
    return (<div className="p-10">
      ...
      <div className="space-y-3">
        {events.map((e) => {
            const past = isPast(e);
            return (<div key={e.id} className={`bg-surface border border-border rounded-md p-5 ${past ? 'opacity-60' : ''}`}>
              <div className="flex items-center justify-between mb-4">
                <p className="text-sm font-semibold">
                  {e.title}
                  <span className="text-foreground-muted font-normal ml-2 text-xs">
                    {new Date(e.startsAt).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo', day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}
