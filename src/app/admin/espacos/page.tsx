...
function SpaceForm({ space }: { space?: ReturnType<typeof listSpaces>[number] }) {
  return (
    <form action={saveSpace} className="flex flex-wrap items-end gap-3">
  ...
  return (
    <div className="p-10">
      ...
      <div className="space-y-3">
        {spaces.map((s) => (
          <div key={s.id} className="bg-surface border border-border rounded-md p-5">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-semibold">
                {s.icon} {s.name}
                <span className="text-foreground-muted font-normal ml-2 text-xs">
                  {countPostsBySpace(s.id)} posts • {s.memberCount.toLocaleString('pt-BR')} membros
                </span>
              </p>
