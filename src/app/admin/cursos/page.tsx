...
export default async function AdminCoursesPage() {
    ...
    return (<div className="p-10">
      <div className="mb-8 flex items-end justify-between">
        ...
        <Link href="/admin/cursos/novo" className="bg-primary text-background font-bold px-5 py-2 rounded-sm hover:bg-primary-hover transition-colors text-sm">
          + Nova trilha
        </Link>
      </div>
      ...
      <div className="space-y-3">
        {(await Promise.all(courses.map(async (c) => {
            ...
            return (<div key={c.id} className="flex flex-wrap items-center gap-4 p-4 bg-surface border border-border rounded-md">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {!c.isPublished && (<span className="text-[9px] uppercase tracking-widest font-bold text-foreground-muted border border-border px-1.5 py-0.5 rounded-sm flex-shrink-0">
                      Rascunho
                    </span>)}
                  {c.featured && (<span className="text-[9px] uppercase tracking-widest font-bold text-primary border border-primary/30 px-1.5 py-0.5 rounded-sm flex-shrink-0">
                      Destaque
                    </span>)}
                </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <form action={togglePublishCourse.bind(null, c.id)}>
                  <button type="submit" className={c.isPublished
                ...
                <Link href={`/admin/cursos/${c.id}`} className="text-xs border border-border px-3 py-1.5 rounded-sm text-foreground hover:border-primary/50 transition-colors">
                  Editar
                </Link>
