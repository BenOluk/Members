    ...
    return (<div className="min-h-screen">
      ...
      <main className="max-w-3xl mx-auto px-6 py-10 pb-20">
        <header className="mb-8 flex items-end justify-between">
          ...
          {hasUnread && (<form action={markAllNotificationsRead}>
        ...
        {list.length === 0 ? (<p className="text-foreground-muted text-center py-20">Caixa vazia. O silêncio também fala.</p>) : (<ul className="space-y-2">
            {list.map((n) => {
                const body = (<div className={`flex items-start gap-4 p-4 rounded-md border ${n.read ? 'bg-surface border-border' : 'bg-primary/5 border-primary/30'}`}>
