    ...
    return (<div className="min-h-screen">
      ...
      <main className="max-w-4xl mx-auto px-6 py-10 pb-20">
        <header className="mb-10">
          ...
          <p className="text-foreground-muted mt-2">Registros das trilhas que você concluiu. Abra para imprimir ou salvar em PDF.</p>
        </header>

        {certs.length === 0 ? (<div className="rounded-lg border border-border bg-surface p-10 text-center">
            ...
            {certs.map((c) => <div key={c.id}><CertificateCard certificate={c}/><Link className="button-secondary mt-4" href={`/certificates/${c.credentialCode}`}>Abrir certificado →</Link></div>)}
          </div>)}
      </main>
    </div>);
}
