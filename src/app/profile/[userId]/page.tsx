...
import { listVisiblePosts } from '@/core/application/community';
...
export default async function ProfilePage({ params }: PageProps) {
    ...
    const snap = (await getUserProgress(userId));
    if (!snap || snap.user.status === 'suspended')
        notFound();
    const { user, badges, level, nextLevel, xpToNext } = snap;
    const isOwnProfile = currentUser.id === user.id;
    const isFollowing = currentUser.followingUserIds.includes(user.id);
    const enrolled = (await Promise.all((isOwnProfile ? user.enrolledCourseIds : [])
        .map(async (id) => (await getCourseById(id))))).filter((c): c is NonNullable<typeof c> => Boolean(c))
        .map((course) => ({ course, progress: getCourseProgress(user, course) }));
    const userPosts = (await listVisiblePosts(currentUser)).filter((p) => p.authorId === user.id);
    ...
    return (<div className="min-h-screen">
      ...
      <section className="relative">
        ...
        <div className="relative max-w-5xl mx-auto px-6 pt-10 pb-8">
          <div className="flex flex-col md:flex-row items-start md:items-end gap-6">
            ...
            {!isOwnProfile && (<form action={toggleFollow.bind(null, user.id)}>
                <button type="submit" className={isFollowing
      ...
      <main className="max-w-5xl mx-auto px-6 pb-20 space-y-10">
        ...
        <section>
          ...
          {badges.length === 0 ? (<p className="text-foreground-muted text-sm">Sem badges por enquanto.</p>) : (<div className="flex flex-wrap gap-6 p-6 bg-surface border border-border rounded-lg">
        ...
        <section>
          ...
          {enrolled.length === 0 ? (<p className="text-foreground-muted text-sm">Nenhuma trilha em andamento.</p>) : (<div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {enrolled.map(({ course, progress }) => (<CourseCard key={course.id} course={course} variant="compact" progress={progress}/>))}
            </div>)}
        </section>
        ...
        <section>
          ...
          {userPosts.length === 0 ? (<p className="text-foreground-muted text-sm">Sem posts ainda.</p>) : (<div className="space-y-4">
