          ...
          <section className="mb-12">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {inProgress.map(({ course, progress }) => (
                <Link
                  key={course.id}
                  href={`/course/${course.id}${progress.nextLessonId ? `?l=${progress.nextLessonId}` : ''}`}
                  className="group flex gap-4 p-4 bg-surface border border-border rounded-md hover:border-primary/50 transition-colors"
                >
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {completed.map(({ course, enrollment }) => (
                <Link
                  ...
                  className="group p-4 bg-surface border border-primary/30 rounded-md hover:border-primary/60 transition-colors"
                >
