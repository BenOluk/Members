      ...
      <main className="max-w-5xl mx-auto px-6 py-10 pb-20">
        <header className="mb-10">
          <form action="/search" className="mb-4">
            <input type="search" name="q" defaultValue={q} autoFocus placeholder="Buscar cursos, pessoas, posts..." className="w-full bg-surface border border-border rounded-full px-6 py-3 text-lg focus:outline-none focus:border-primary"/>
          </form>
        ...
        {results.courses.length > 0 && (<section className="mb-10">
        ...
        {results.users.length > 0 && (<section className="mb-10">
            <h2 className="text-xl font-heading font-semibold mb-4">Pessoas</h2>
            <ul className="grid md:grid-cols-2 gap-3">
              {results.users.map((u) => (<li key={u.id}>
        ...
        {results.spaces.length > 0 && (<section className="mb-10">
            <h2 className="text-xl font-heading font-semibold mb-4">Espaços</h2>
            <ul className="grid md:grid-cols-2 gap-3">
              {results.spaces.map((s) => (<li key={s.id}>
        ...
        {q && total === 0 && (<p className="text-foreground-muted text-center py-20">Nada encontrado.</p>)}
      </main>
    </div>);
}
