...
export async function createPost(formData: FormData): Promise<void> {
    ...
    if (!content || content.length > 5000 || title.length > 200 || !await consumeLimit(`post:${user.id}`, 10, 60000))
        return;
    const space = (await getSpaceById(spaceId));
    if (!space || !canUserAccessSpace(user, space))
        return;
    (await communityRepo.createPost({
        spaceId,
        authorId: user.id,
...
export async function toggleLikePost(postId: string): Promise<void> {
    const user = await requireUser();
    if (!await consumeLimit(`like:${user.id}`, 120, 60000)) return;
    ...
    if (!space || !canUserAccessSpace(user, space))
        return;
    (await communityRepo.toggleLike(postId, user.id));
...
export async function addComment(postId: string, formData: FormData): Promise<void> {
    const user = await requireUser();
    const content = String(formData.get('content') ?? '').trim();
    if (!content || content.length > 2000 || !await consumeLimit(`comment:${user.id}`, 30, 60000))
        return;
    const post = (await getPostById(postId));
    if (!post)
        return;
    ...
    if (!space || !canUserAccessSpace(user, space))
        return;
    (await communityRepo.addComment({ postId, authorId: user.id, content }));
...
export async function togglePinPost(postId: string): Promise<void> {
    ...
    if (!post)
        return;
    (await communityRepo.setPinned(postId, !post.pinned));
    revalidateCommunity(post.spaceId);
}
export async function deletePost(postId: string): Promise<void> {
    ...
    if (post.authorId !== user.id && !isModerator(user))
        return;
    (await communityRepo.removePost(postId));
    revalidateCommunity(post.spaceId);
}
export async function toggleFollow(userId: string): Promise<void> {
    const user = await requireUser();
    if (user.id === userId || !(await usersRepo.getById(userId)))
        return;
    (await usersRepo.toggleFollow(user.id, userId));
